    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        @if (!$formAvailable)
            ...
            <div class="card animate-scale-in">
                ...
                <div class="p-10 sm:p-16 text-center">
                    ...
                    <p class="text-lg text-gray-500 max-w-md mx-auto leading-relaxed">Este formulario no está disponible actualmente. Puede que sea privado o que el enlace sea incorrecto.</p>
        ...
        @elseif ($successMessage)
            ...
            <div class="card animate-scale-in">
                ...
                <div class="p-10 sm:p-20 text-center">
                    <div class="inline-flex items-center justify-center w-28 h-28 rounded-full bg-emerald-50 mb-8 animate-bounce shadow-inner">
        ...
        @else
            ...
            <div class="card mb-8 animate-slide-up">
                ...
                <div class="px-8 py-10 sm:px-12 sm:py-12">
                    <h1 class="text-3xl sm:text-4xl font-black text-gray-900 tracking-tight leading-tight">{{ $form->title }}</h1>
                    @if ($form->description)
                        <div class="mt-4 prose prose-indigo max-w-none">
                            <p class="text-lg text-gray-500 leading-relaxed">{{ $form->description }}</p>
                        </div>
                    @endif
                    <div class="mt-8 flex items-center gap-2 text-sm font-semibold text-gray-400 border-t border-gray-100 pt-6">
            ...
            <form wire:submit="submit" class="space-y-8">
                {{-- Honeypot --}}
                <div class="absolute" style="left: -9999px; opacity: 0; height: 0; width: 0; overflow: hidden;" aria-hidden="true" tabindex="-1">
                    ...
                    <div class="card relative animate-slide-up" style="animation-delay: {{ ($loop->index + 1) * 0.1 }}s">
                        ...
                        <div class="px-8 py-10 sm:px-10 sm:py-10">
                            <div class="flex items-start gap-6">
                                <span class="flex-shrink-0 mt-0.5 w-10 h-10 rounded-2xl bg-indigo-50 flex items-center justify-center text-sm font-bold text-indigo-600 shadow-sm border border-indigo-100">
                                    {{ $loop->iteration }}
                                </span>
                                <div class="flex-1 min-w-0">
                                    ...
                                    @if ($question->type === 'text')
                                        <div class="relative group">
                                            <textarea wire:model="answers.question_{{ $question->id }}" rows="4"
                                                      class="block w-full rounded-2xl border-2 border-gray-100 bg-gray-50/30 px-6 py-5 text-lg
                                                             ...
                                                             transition-all duration-300 placeholder:text-gray-400 resize-y"
                                                      placeholder="Escribe tu respuesta aquí..."></textarea>
                                            <div class="absolute bottom-4 right-4 text-xs font-bold text-gray-300 pointer-events-none group-focus-within:text-brand-400">TEXTO</div>
                                        </div>
                                        ...
                                        <div class="grid grid-cols-1 gap-4">
                                            @foreach ($question->options as $option)
                                                ...
                                                <div wire:click="$set('answers.question_{{ $question->id }}', {{ $option->id }})"
                                                    ...
                                                    <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all duration-300 {{ $selected ? 'border-brand-500 bg-brand-500 shadow-[0_0_10px_rgba(79,70,229,0.4)]' : 'border-gray-300 bg-white' }}">
                                                        <div class="w-2.5 h-2.5 rounded-full bg-white transition-transform duration-300 {{ $selected ? 'scale-100' : 'scale-0' }}"></div>
                                                    </div>
                                        ...
                                        <div class="grid grid-cols-1 gap-4">
                                            @foreach ($question->options as $option)
                                                ...
                                                <div wire:click="toggleCheckbox({{ $question->id }}, {{ $option->id }})"
                                                    ...
                                                    <div class="w-6 h-6 rounded-lg border-2 flex items-center justify-center flex-shrink-0 transition-all duration-300 {{ $checked ? 'bg-brand-500 border-brand-500 shadow-[0_0_10px_rgba(79,70,229,0.4)]' : 'border-gray-300 bg-white' }}">
                                                        <svg class="w-4 h-4 text-white transition-opacity duration-300 {{ $checked ? 'opacity-100' : 'opacity-0' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"/></svg>
                                                    </div>
                                                    <span class="text-lg font-semibold select-none">{{ $option->option_text }}</span>
                                                </div>
                                            @endforeach
                                    ...
                                    @elseif ($question->type === 'select')
                                        <div class="relative">
                                            <select wire:model="answers.question_{{ $question->id }}"
                                                    class="block w-full rounded-2xl border-2 border-gray-100 bg-gray-50/30 px-6 py-5 text-lg appearance-none
                                                           focus:border-brand-500 focus:ring-4 focus:ring-brand-500/5 focus:bg-white
                                    ...
                                    @error("answers.question_{$question->id}")
                                        <div class="mt-6 flex items-center gap-3 p-4 bg-red-50 text-red-700 rounded-2xl border border-red-100 animate-slide-up">
                ...
                <div class="flex flex-col items-center gap-6 pt-12">
                    <button type="submit"
                            class="w-full sm:max-w-md inline-flex items-center justify-center gap-4 px-10 py-6
                    ...
                    <p class="text-xs font-bold text-gray-300 uppercase tracking-widest">Tus respuestas se guardan de forma segura</p>
                </div>
            </form>
        @endif
        ...
        <div class="mt-20 pt-10 border-t border-gray-100 text-center">
            <p class="text-sm font-bold text-gray-400 tracking-wide uppercase">
                Hecho con <span class="text-brand-500">Formularios</span> &bull; 2026
            </p>
        </div>
    </div>
</div>
