    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">Bienvenido, {{ auth()->user()->name }}</p>
            </div>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
                <div class="stat-card">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900">{{ $formsCount }}</p>
                        <p class="text-sm text-gray-500">Formularios</p>
                    </div>
                </div>
                <div class="stat-card" style="animation-delay: 0.1s">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900">{{ $responsesCount }}</p>
                        <p class="text-sm text-gray-500">Respuestas totales</p>
                    </div>
                </div>
                <div class="stat-card" style="animation-delay: 0.2s">
                    ...
                    <div>
                        <p class="text-2xl font-bold text-gray-900">{{ $questionsCount }}</p>
                        <p class="text-sm text-gray-500">Preguntas creadas</p>
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                @if ($recentForms->isEmpty())
                    <div class="empty-state">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900 mb-1">No tienes formularios</h3>
                        <p class="text-gray-500 mb-6 max-w-sm">Crea tu primer formulario y empieza a recolectar respuestas.</p>
                        ...
                        @foreach ($recentForms as $form)
                            <div class="px-6 py-4 flex items-center justify-between hover:bg-gray-50/50 transition-colors duration-150">
                                <div class="flex items-center gap-4 min-w-0">
                                    <div class="flex-shrink-0 w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center text-brand-600 font-bold">
                                        {{ strtoupper(substr($form->title, 0, 2)) }}
                                    </div>
                                    <div class="min-w-0">
                                        <a href="{{ route('forms.show', $form) }}" class="text-sm font-semibold text-gray-900 hover:text-brand-600 transition-colors truncate block">
                                            {{ $form->title }}
                                        </a>
                                        <div class="flex items-center gap-3 mt-1">
                                            <span class="text-xs text-gray-400">
                                                <span class="font-medium text-gray-500">{{ $form->questions_count }}</span> preguntas
                                            </span>
                                            <span class="text-gray-300">&middot;</span>
                                            <span class="text-xs text-gray-400">
                                                <span class="font-medium text-gray-500">{{ $form->responses_count }}</span> respuestas
                                            </span>
                                            <span class="text-gray-300">&middot;</span>
                                            @if ($form->is_public)
